...
describe('useHistoryView', () => {
  ...
  it('setMode 更新并持久化', () => {
    ...
    expect(window.localStorage.getItem('acpfe.historyView')).toBe(JSON.stringify({ mode: 'marked' }))
  })

  it('脏 localStorage 值 → 回退 workspace', async () => {
    window.localStorage.setItem('acpfe.historyView', JSON.stringify({ mode: 'bogus' }))
    expect((await freshStore()).getState().mode).toBe('workspace')
  })

  it('marked 值被正确读取', async () => {
    window.localStorage.setItem('acpfe.historyView', JSON.stringify({ mode: 'marked' }))
